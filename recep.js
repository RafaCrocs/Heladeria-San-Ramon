...
async function iniciarEscuchaPedidos() {
    const pedidosRef = ref(database, 'SanRamon/' + 'pedidosEnProceso');
...
window.completarPedido = async function(boton) {
    ...
    if (snapshot.exists()) {
        ...
        const acumuladoPath = `SanRamon/estadisticas_ventas/${diaClave}`;
        ...
        const completadoRef = ref(database, 'SanRamon/' + 'pedidosCompletados/' + diaClave + '/' + pedidoId);
