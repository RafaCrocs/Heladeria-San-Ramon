...
function renderReposteria(productos, contenedor) {
    const section = document.getElementById(contenedor);
    section.innerHTML = productos.map(p => `
        <label class="caja" for="${p.nombre}">
            <p class="nombreProductos">${p.nombre}</p>
            <div class="imagenPedido ${p.imagen}"></div>
            <button class="btnAgregar" id="${p.nombre}" onclick="agregarReposteria('${p.nombre}')">AGREGAR</button>
        </label>
        `
    ).join('');
}
...
function cerrarVentanaEmergente() {
    ...
    document.getElementById("tituloVentanaEmergente").innerHTML = "";
...
function enviarPedido() {
    ...
    const pedidoRef = ref(database, 'SanRamon/' + 'pedidosEnProceso');
...
renderReposteria(productos.postres, "contenedorPostres");
...
window.agregarReposteria = agregarReposteria;
...
function agregarReposteria(nombre) {
    bebidaEnProgreso.bebida = nombre;
    carrito.push(bebidaEnProgreso);
    restablecerBebidaEnProgreso();
    cerrarVentanaEmergente();
